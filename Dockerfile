...
RUN python -m venv /opt/german-protest-registrations-env
ENV PATH /opt/german-protest-registrations-env/bin:$PATH
ENV VIRTUAL_ENV /opt/german-protest-registrations-env

# Set the working directory.
WORKDIR /workspaces/german-protest-registrations/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/german-protest-registrations/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/german_protest_registrations/ && touch src/german_protest_registrations/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/german-protest-registrations/
...
ENTRYPOINT ["/opt/german-protest-registrations-env/bin/poe"]
CMD ["app"]

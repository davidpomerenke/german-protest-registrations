...
df = pd.read_csv("../data/processed/german_protest_registrations.csv", parse_dates=["event_date"])
df = df[df["event_date"].dt.year >= 2019]
# df = df[df["event_date"].dt.year == 2022]
df = df[~df["city"].isin(["Freiburg", "Erfurt", "Karlsruhe", "Köln", "Wuppertal", "Saarbrücken", "Dresden", "Kiel"])]
len(df)
```
